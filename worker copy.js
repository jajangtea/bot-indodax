...
const BASE_TP = 1.8;
const BASE_SL = 1.2;
const MIN_VOL_24H = 100_000_000; // 100 Juta (Sesuai settingan kamu)
...
async function executeBuy(pair, price, score) {
    // 1. Cek slot posisi TANPA panggil API lagi (Hemat waktu)
    ...
    // 2. Gunakan saldo yang sudah kita punya di state.cachedBalance
    const capital = Math.floor(state.cachedBalance * 0.75);


    // Margin aman: Indodax minimal 10rb, kita set 11rb agar aman dari fee
    if (capital < 11000) {
        console.log(`\x1b[31m[SKIP] Dana 25% (Rp ${capital}) di bawah limit minimal Indodax.\x1b[0m`);
        return;
    }

    console.log(`\x1b[32m[ORDER] 🚀 MENEMBAK ${pair.toUpperCase()} senilai Rp ${capital.toLocaleString()}...\x1b[0m`);

    // 3. Eksekusi Trade
    const res = await privateReq("trade", {
        pair,
        type: "buy",
        price: price, // Tetap gunakan price last
        idr: capital
    });
    ...
    if (res.success) {
        state.positions[pair] = {
            buyPrice: price,
            capital: capital,
            tp: price * (1 + BASE_TP / 100),
            sl: price * (1 - BASE_SL / 100),
            coin: pair.split("_")[0],
            isTrailing: false
        };
        console.log(`\x1b[42m\x1b[30m SUCCESS \x1b[0m Berhasil beli ${pair.toUpperCase()}!`);
        tg(`📥 *BELI BERHASIL*\nPair: ${pair.toUpperCase()}\nHarga: ${price}\nModal: Rp ${capital.toLocaleString()}`);

        // Update saldo lokal agar tidak beli koin lain dengan uang yang sudah terpakai
        state.cachedBalance -= capital;
    } else {
        console.log(`\x1b[31m[REJECTED] Indodax menolak: ${res.error}\x1b[0m`);
        // Jika error karena saldo, paksa update saldo di scan berikutnya
...
async function managePositions(pair, currentPrice) {
    ...
    const gain = ((currentPrice - pos.buyPrice) / pos.buyPrice) * 100;
    if (gain > 0.8 && !pos.isTrailing) {
    ...
    try {
        ...
        if (topWatch.length === 0) {
            ...
            topWatch.forEach(r => {
                ...
                if (r.score >= 6) status = "\x1b[31m🚀 BUY ZONE\x1b[0m";
        ...
        const candidates = currentRanked.filter(r => r.score >= 6);
